...
export class AuthguardService implements CanActivate{
  ...
  constructor(private http: HttpClient, public router: Router, public route : ActivatedRoute, private weighingAPI : WeighingService) { 
  ...
  // --Management
  item_023 = { title: 'ユーザーの管理', url:'' , icon: 'people' , children :[]};
  item_002=  { title: '会社のユーザー', url: '/mng-users', icon: 'people'};
  item_003 = { title: '検査 官', url: '/mng-inspectors', icon: 'person'};

  // --Inspection
  item_004 = { title: 'スキャナ', url: '/qrscanner', icon: 'qr-scanner'};
  item_005 = { title: '着信車', url: '/vehicles', icon: 'bus'};
  item_006 = { title: '検査ログ', url: '/inspection-logs', icon: 'paper'};
  ...
  canActivate(route: ActivatedRouteSnapshot,state : RouterStateSnapshot):boolean {
    ...
    if(this.getSessionData()){
        ...
        //  || target_routepath =='/weighing-detail'
  ...
  setAuthority() {
      ...
      if(this.customer_type === 1) {
          ...
          if(this.is_customer_admin) {
              this.item_023.children.push(this.item_002);
              this.item_023.children.push(this.item_003);
              this.authorized_items.push(this.item_023);
              this.authorized_paths.push({url:this.item_002.url});
              this.authorized_paths.push({url:this.item_003.url});
              // this.authorized_items.push(this.item_002);
              // this.authorized_items.push(this.item_003);
          ...
          // this.authorized_paths.push({url:'/report-discreteship'});
...
// public isProviderID;
// public provider_prefix = localStorage.getItem('provider_prefix') || '';
// public provider_name = localStorage.getItem('provider_name') || '船積みネット';
// public login_username = localStorage.getItem('username') || 'a';
// public user_type = localStorage.getItem('authority') || '';
// public auth_id = '';
// public auth_user = '';
